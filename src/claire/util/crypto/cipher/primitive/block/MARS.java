...
import claire.util.crypto.cipher.key.block.KeyMARS;
import claire.util.crypto.rng.RandUtils;
import claire.util.memory.Bits;
import claire.util.standards.crypto.ISymmetric;

public class MARS 
	   implements ISymmetric<KeyMARS> {
	...
	private static final int[] MS = new int[] {
	...
	public MARS(KeyMARS mars)
	{
		this.setKey(mars);
	}
	
	public KeyMARS getKey()
	{
		return src;
	}

	public void setKey(KeyMARS tk)
	{
		src = tk;
		int[] raw = tk.getInts();
		int[] t = new int[15];
		System.arraycopy(raw, 0, t, 0, raw.length);
		t[raw.length] = raw.length;
		...
		for(int i = 0; i < 4; i++) {
			t[ 0] ^= x++;
			t[ 0] ^= Bits.rotateLeft(t[8 ] ^ t[13], 3);
			t[ 1] ^= x++;
			t[ 1] ^= Bits.rotateLeft(t[9 ] ^ t[14], 3);
			t[ 2] ^= x++;
			t[ 2] ^= Bits.rotateLeft(t[10] ^ t[ 0], 3);
			t[ 3] ^= x++;
			t[ 3] ^= Bits.rotateLeft(t[11] ^ t[ 1], 3);
			t[ 4] ^= x++;
			t[ 4] ^= Bits.rotateLeft(t[12] ^ t[ 2], 3);
			t[ 5] ^= x++;
			t[ 5] ^= Bits.rotateLeft(t[13] ^ t[ 3], 3);
			t[ 6] ^= x++;
			t[ 6] ^= Bits.rotateLeft(t[14] ^ t[ 4], 3);
			t[ 7] ^= x++;
			t[ 7] ^= Bits.rotateLeft(t[ 0] ^ t[ 5], 3);
			t[ 8] ^= x++;
			t[ 8] ^= Bits.rotateLeft(t[ 1] ^ t[ 6], 3);
			t[ 9] ^= x++;
			t[ 9] ^= Bits.rotateLeft(t[ 2] ^ t[ 7], 3);
			t[10] ^= x++;
			t[10] ^= Bits.rotateLeft(t[ 3] ^ t[ 8], 3);
			t[11] ^= x++;
			t[11] ^= Bits.rotateLeft(t[ 4] ^ t[ 9], 3);
			t[12] ^= x++;
			t[12] ^= Bits.rotateLeft(t[ 5] ^ t[10], 3);
			t[13] ^= x++;
			t[13] ^= Bits.rotateLeft(t[ 6] ^ t[11], 3);
			t[14] ^= x++;
			t[14] ^= Bits.rotateLeft(t[ 7] ^ t[12], 3);
			for(int j = 0; j < 4; j++) {
				t[ 0] = Bits.rotateLeft(t[ 0] + SBOX[t[14] & 511], 9);
				t[ 1] = Bits.rotateLeft(t[ 1] + SBOX[t[ 0] & 511], 9);
				t[ 2] = Bits.rotateLeft(t[ 2] + SBOX[t[ 1] & 511], 9);
				t[ 3] = Bits.rotateLeft(t[ 3] + SBOX[t[ 2] & 511], 9);
				t[ 4] = Bits.rotateLeft(t[ 4] + SBOX[t[ 3] & 511], 9);
				t[ 5] = Bits.rotateLeft(t[ 5] + SBOX[t[ 4] & 511], 9);
				t[ 6] = Bits.rotateLeft(t[ 6] + SBOX[t[ 5] & 511], 9);
				t[ 7] = Bits.rotateLeft(t[ 7] + SBOX[t[ 6] & 511], 9);
				t[ 8] = Bits.rotateLeft(t[ 8] + SBOX[t[ 7] & 511], 9);
				t[ 9] = Bits.rotateLeft(t[ 9] + SBOX[t[ 8] & 511], 9);
				t[10] = Bits.rotateLeft(t[10] + SBOX[t[ 9] & 511], 9);
				t[11] = Bits.rotateLeft(t[11] + SBOX[t[10] & 511], 9);
				t[12] = Bits.rotateLeft(t[12] + SBOX[t[11] & 511], 9);
				t[13] = Bits.rotateLeft(t[13] + SBOX[t[12] & 511], 9);
				t[14] = Bits.rotateLeft(t[14] + SBOX[t[13] & 511], 9);
			}
			for(int j = 0; j < 10; j++)
				key[p++] = t[++k > 14 ? k = 0 : k];
		}
		...
		for(int i = 5; i < 36; i += 2) {
			...
			if(p2) 
				l = 2;
			for(int j = 2; j < 32; j++) {
				boolean cur = Bits.getBit(k, j);
				if(p2 && cur) {
					l++;
					Bits.setBit(prem, j - 1, true);
					if(l > 10)
						m |= prem;
				} else {
					prem = 0;
			...
			key[i] ^= m & Bits.rotateLeft(ms, key[i - 1] & 31);
		}		
	}

	public void encryptBlock(byte[] block, int start)
	{
		int A = Bits.intFromBytes(block, start     ) + key[0],
			B = Bits.intFromBytes(block, start + 4 ) + key[1],
			C = Bits.intFromBytes(block, start + 8 ) + key[2],
			D = Bits.intFromBytes(block, start + 12) + key[3],
			T, R, M, L, T2;
		...
		for(int i = 0; i < 8; i++) {
			T2 = key[kpos++];
			R = T = Bits.rotateLeft(A, 13);
			R *= key[kpos++];
			R = Bits.rotateLeft(R, 10);
			M = A + T2;
			T2 = Bits.rotateRight(R, 5);
			M = Bits.rotateLeft(M, T2 & 31);
			L = Bits.rotateLeft((SBOX[M & 511] ^ T2) ^ R, R & 31);
			B += L;
			C += M;
			D ^= R;
			A = B;
	...
	{
		int A = Bits.intFromBytes(block, start0     ) + key[0],
			B = Bits.intFromBytes(block, start0 + 4 ) + key[1],
			C = Bits.intFromBytes(block, start0 + 8 ) + key[2],
			D = Bits.intFromBytes(block, start0 + 12) + key[3],
			T, R, M, L, T2;
	...
	{
		int A = Bits.intFromBytes(block, start     ) + key[36],
			B = Bits.intFromBytes(block, start + 4 ) + key[37],
			C = Bits.intFromBytes(block, start + 8 ) + key[38],
			D = Bits.intFromBytes(block, start + 12) + key[39],
			T, R, M, L, T2;

		T = Bits.rotateRight(D, 24);
		D = C;
		C = B;
		B = A;
		A = T;	
		...
		for(int i = 0; i < 8; i++) {
			...
			C = B;
			B = A;
			A = Bits.rotateRight(T, 13);
			R = Bits.rotateLeft(T * key[kpos--], 10);
			T2 = Bits.rotateRight(R, 5);
			M = Bits.rotateLeft(A + key[kpos--], T2 & 31);
			L = Bits.rotateLeft((SBOX[M & 511] ^ T2) ^ R, R & 31);
			B ^= R;
			C -= M;
			D -= L;
	...
	{
		int A = Bits.intFromBytes(block, start0     ) + key[36],
			B = Bits.intFromBytes(block, start0 + 4 ) + key[37],
			C = Bits.intFromBytes(block, start0 + 8 ) + key[38],
			D = Bits.intFromBytes(block, start0 + 12) + key[39],
			T, R, M, L, T2;
	...
	public static final int test()
	{
		...
		final int[] ints2 = new int[7];
		RandUtils.fillArr(ints1);
		RandUtils.fillArr(ints2);
		KeyMARS a1 = new KeyMARS(ints1);
		KeyMARS a2 = new KeyMARS(ints2);
		MARS aes = new MARS(a1);
		return ISymmetric.testSymmetric(aes, a2);
	}

	public KeyFactory<KeyMARS> keyFactory()
	{
		return KeyMARS.factory;
	}
