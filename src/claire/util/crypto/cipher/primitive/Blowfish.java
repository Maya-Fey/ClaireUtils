...
public class Blowfish implements ISymmetric<KeyBlowfish> {
	...
	private static final int[][] SBOXORIGIN = 
			{
	        	{
	...
	private final int[][] SCUBE = new int[4][256];
	private final int[] SKEY = new int[18];
	...
	private int F(int A)
	{
		int h = SCUBE[0][A >> 24 & 0xFF] + SCUBE[1][A >> 16 & 0xFF];
	    return (h ^ SCUBE[2][A >> 8 & 0xFF]) + SCUBE[3][A & 0xFF];
	}
	
	private void gen(int A, int B, int[] box)
	{
		int size = box.length;
	...
	public void setKey(KeyBlowfish t)
	{
		this.key = t;
		byte[] key = t.getBytes();
		for(int i = 0; i < 4; i++)
			System.arraycopy(SBOXORIGIN[i], 0, SCUBE[i], 0, 256);
		System.arraycopy(KEYORIGIN, 0, SKEY, 0, 18);
		...
		for(int i = 0; i < 18; i++)
		{
			...
			if((len - index) >= 4)
			{
				data = ((((int) key[index++] & 0xFF)) |
				   (((int) key[index++] & 0xFF) << 8) |
				  (((int) key[index++] & 0xFF) << 16) |
				  (((int) key[index++] & 0xFF) << 24));
				if((len - index) == 0)
					index = 0;
			} else {
			data |= (int) (key[index++] & 0xFF);
			if((len - index) == 0)
				index = 0;
			data |= (int) ((key[index++] << 8) & 0xFF);
			if((len - index) == 0)
				index = 0;
			data |= (int) ((key[index++] << 16) & 0xFF);
			if((len - index) == 0)
				index = 0;
			data |= (int) ((key[index++] << 24) & 0xFF);
			if((len - index) == 0)
				index = 0;
			}
			SKEY[i] ^= data;
		...
		gen(       0,                  0,     SKEY);
		gen(SKEY[16],           SKEY[17], SCUBE[0]);
		gen(SCUBE[0][254], SCUBE[0][255], SCUBE[1]);
		gen(SCUBE[1][254], SCUBE[1][255], SCUBE[2]);
	...
	public void decryptBlock(byte[] block, int start)
	{
		...
		Bits.intToBytes(B, block, start + 0);
		Bits.intToBytes(A, block, start + 4);
	}
	...
	public void decryptBlock(byte[] block, int start0, byte[] out, int start1)
	{
		...
		Bits.intToBytes(B, out, start1 + 0);
		Bits.intToBytes(A, out, start1 + 4);
	}
	...
	public KeyBlowfish newKey(IRandom rand)
	{
		byte[] bytes = new byte[this.key.keySize()];
