...
public class SEED 
	   implements ISymmetric<KeySEED> {
	
	private static final int[][] SCUBE =
    {
		{
	...
	public void setKey(KeySEED t)
	{
		...
		for(int i = 0; i < 16;)
		{
			KEY[j++] = subBytes(work[0] + work[1] - KS[i  ]);
			KEY[j++] = subBytes(work[2] + work[3] - KS[i++]);
			
			in[0] = Bits.rotateRight(in[0], 8);
			Bits.longsToInts(in, 0, work, 0, 1);
			
			KEY[j++] = subBytes(work[0] + work[1] - KS[i  ]);
			KEY[j++] = subBytes(work[2] + work[3] - KS[i++]);
			
			in[0] = Bits.rotateLeft(in[1], 8);
			Bits.longsToInts(in, 1, work, 2, 1);
		}
	}
    ...
    {
        return subBytes(subBytes((A ^ B) ^ (C ^ D)) + (A ^ B));
    }
	
	private static int F2(int A, int B, int C, int D)
    {
        return subBytes(F1(A, B, C, D) + subBytes((A ^ B) ^ (C ^ D)));
    }
	
	private static long FM(int A, int B, long G)
    {
        int C = (int) (G >> 32);
        int D = (int) G;
        int E = F2(C, A, D, B);
        ...
        return ((long) F << 32) | (E & 0xFFFFFFFFL);
    }
	
	/*
	 * Fuck you just because I'm lazy.
	 */
	private static long IFM(int A, int B, long G)
    {
        return FM(B, A, G);
    }
	...
	public void decryptBlock(byte[] block, int start)
	{
		...
		{
			T = B;
			B = A ^ IFM(KEY[k--], KEY[k--], B);
			A = T;
		}
		
		Bits.longToBytes(B, block, start + 0);
		Bits.longToBytes(A, block, start + 8);
	}
	...
	public KeyFactory<KeySEED> keyFactory()
	{
		return KeySEED.factory;
	}
