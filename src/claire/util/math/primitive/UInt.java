...
public class UInt 
	...
	private final void fastdiv(final int[] divisor, final int dlen, int vlen)
	{
		...
		if(shift > 0) {
			MathHelper.shiftArrayRightBE(divisor, shift, dlen);
			MathHelper.shiftArrayRightBE(divd, shift, vlen);
		}
		...
		for(int i = vlen - dlen; i >= 0; i--)
		{
			...
			for(int j = 0; j < dlen; j++) {	
				carry += ((divisor[j] & 0xFFFFFFFFL) * mul);
				int low = (int) carry;
				carry >>>= 32;
				if(Bits.u_greaterThan(low, divd[pos]))
					carry++;
				divd[pos] = divd[pos++] - low;
			}
	...
	private final void fastmod(final int[] divisor, final int dlen, int vlen)
	{
		...
		for(int i = vlen - dlen; i >= 0; i--)
		{
			...
			if(Bits.u_greaterThan(carry, prev & 0xFFFFFFFFL)) {
				long c2 = 0;
				pos = i;
				for(int j = 0; j < dlen; j++) {
					if(pos >= 0) {
