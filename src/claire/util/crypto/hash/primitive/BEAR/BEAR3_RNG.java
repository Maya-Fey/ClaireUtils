...
public final class BEAR3_RNG 
			 implements IRandom {
	
	/*
	 * It's a tactical cube. You're screwed
	 */
	private static final int[][] BORG = 
	{
		{
	...
	private int pos, statepos;
	
	public BEAR3_RNG(int[] seed)
	{
		this.seed(seed);
	}
	
	public BEAR3_RNG(byte[] bytes)
	{
		seed(bytes);
	}
	
	public BEAR3_RNG(byte[] bytes, int start, int len)
	{
		seed(bytes, start, len);
	}

	public void seed(byte[] bytes)
	{
		seed(bytes, 0, bytes.length);
	}
	
	public void seed(byte[] bytes, int start, int len)
	{
		pos = 0;
		statepos = 0;
		int[] temp = new int[len];
		for(int i = 0; i < len; i++)
			temp[i] = BORG[i & 3][bytes[start++] & 0xFF];
		plant = new int[(temp.length >> 2) + 1];
		int A = BORG[0][0], 
			B = BORG[1][0], 
			C = BORG[2][0], 
			D = BORG[3][0];
		...
		{
			D ^= A ^ temp[i] + BORG[3][B & 0xFF];
			C ^= B ^ temp[i] + BORG[2][A & 0xFF];
			B ^= C ^ temp[i] + BORG[1][D & 0xFF];
			A ^= D ^ temp[i] + BORG[0][C & 0xFF];
			if((i & 3) == 3) 
				plant[j++] = (~A ^ B) + (C ^ ~D);
		}
		plant[j] = A ^ B ^ C ^ D;
		STATE = Bits.intsToBytes(new int[] { A, B, C, D });
	}
	...
	public void seed(int[] seeds)
	{
		pos = 0;
		statepos = 0;
		byte[] bytes = Bits.intsToBytes(seeds);
		int[] temp = new int[bytes.length];
		for(int i = 0; i < bytes.length; i++)
			temp[i] = BORG[i & 3][bytes[i] & 0xFF];
		plant = new int[temp.length >> 2];
		int A = BORG[0][0], 
			B = BORG[1][0], 
			C = BORG[2][0], 
			D = BORG[3][0];
		...
		for(int i = 0; i < temp.length; i++)
		{
			D ^= A ^ temp[i] + BORG[0][i & 0xFF];
			C ^= B ^ temp[i] + BORG[1][i & 0xFF];
			B ^= C ^ temp[i] + BORG[2][i & 0xFF];
			A ^= D ^ temp[i] + BORG[3][i & 0xFF];
			if((i & 3) == 3) 
				plant[j++] = (~A ^ B) + (C ^ ~D);
		}
		STATE = Bits.intsToBytes(new int[] { A, B, C, D });
	...
	public void water(int water)
	{
		...
		{
			plant[incpos()] ^= prev;
			prev ^= Bits.rotateRight(plant[pos], (int) prev & 31);
		}
	}
	
	public void flash(byte photon)
	{
		byte prev = photon;
		for(int i = 0; i < 16; i++)
		{
			STATE[i] ^= prev;
			prev = Bits.rotateLeft(STATE[i], prev & 7);
		}
		statepos = 0;
	}
	...
	private void update()
	{
		...
		C += b3 * C;
		D += b4 * D;
		A ^= Bits.rotateRight(D, 7 );
		B ^=  Bits.rotateLeft(C, 7 );
		C ^= Bits.rotateRight(B, 13);
		D ^=  Bits.rotateLeft(A, 13);
		A += BORG[b4 & 3][b1 & 0xFF];
		B += BORG[b3 & 3][b2 & 0xFF];
		C += BORG[b2 & 3][b3 & 0xFF];
		D += BORG[b1 & 3][b4 & 0xFF];	
