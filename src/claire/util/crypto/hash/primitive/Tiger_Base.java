...
abstract class Tiger_Base<Hash extends Tiger_Base<Hash>>
	...
	private long R1, R2, R3;
	
	private final byte pad;

	public Tiger_Base(byte start)
	{
		super(64, 24);
		pad = start;
		reset();
	...
	public void reset()
	{
        ...
        R3 = 0xF096A5B4C3B2E187L;
	}
	
	private void F1(long in, long mul)
	{
	    R3 ^= in ;
	    R1 -= SCUBE[0][(int)R3 & 0xff] ^ SCUBE[1][(int)(R3 >> 16) & 0xff]
	             ^ SCUBE[2][(int)(R3 >> 32) & 0xff] ^ SCUBE[3][(int)(R3 >> 48) & 0xff];
	    R2 += SCUBE[3][(int)(R3 >> 8) & 0xff] ^ SCUBE[2][(int)(R3 >> 24) & 0xff]
	             ^ SCUBE[1][(int)(R3 >> 40) & 0xff] ^ SCUBE[0][(int)(R3 >> 56) & 0xff];
	    R2 *= mul;
	}

	private void F2(long in, long mul)
	{
	    R1 ^= in ;
	    R2 -= SCUBE[0][(int)R1 & 0xff] ^ SCUBE[1][(int)(R1 >> 16) & 0xff]
	            ^ SCUBE[2][(int)(R1 >> 32) & 0xff] ^ SCUBE[3][(int)(R1 >> 48) & 0xff];
	    R3 += SCUBE[3][(int)(R1 >> 8) & 0xff] ^ SCUBE[2][(int)(R1 >> 24) & 0xff]
	            ^ SCUBE[1][(int)(R1 >> 40) & 0xff] ^ SCUBE[0][(int)(R1 >> 56) & 0xff];
	    R3 *= mul;
	}

	private void F3(long in, long mul)
	{
	    R2 ^= in ;
	    R3 -= SCUBE[0][(int)R2 & 0xff] ^ SCUBE[1][(int)(R2 >> 16) & 0xff]
	            ^ SCUBE[2][(int)(R2 >> 32) & 0xff] ^ SCUBE[3][(int)(R2 >> 48) & 0xff];
	    R1 += SCUBE[3][(int)(R2 >> 8) & 0xff] ^ SCUBE[2][(int)(R2 >> 24) & 0xff]
	            ^ SCUBE[1][(int)(R2 >> 40) & 0xff] ^ SCUBE[0][(int)(R2 >> 56) & 0xff];
	    R1 *= mul;
	}
	    
	
	private static void reschedule(long[] state)
	{
		state[0] -= state[7] ^ 0xA5A5A5A5A5A5A5A5L; 
        state[1] ^= state[0]; 
        state[2] += state[1]; 
        state[3] -= state[2] ^ ((~state[1]) << 19); 
        state[4] ^= state[3]; 
        state[5] += state[4]; 
        state[6] -= state[5] ^ ((~state[4]) >>> 23); 
        state[7] ^= state[6]; 
        state[0] += state[7]; 
        state[1] -= state[0] ^ ((~state[7]) << 19); 
        state[2] ^= state[1]; 
        state[3] += state[2]; 
        state[4] -= state[3] ^ ((~state[2]) >>> 23); 
        state[5] ^= state[4]; 
        state[6] += state[5]; 
        state[7] -= state[6] ^ 0x0123456789ABCDEFL;
	}
	...
	public void processNext(byte[] bytes, int offset)
	{
		...
		long[] IN = Bits.bytesToLongs(bytes);
