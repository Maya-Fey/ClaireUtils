...
public class VariableSInt 
	...
	protected void p_multiply(int[] ints, int len)
	{
		...
		if((len + tlen) > this.length)
			this.build(tlen + len);
		int max = val.length;
		MathHelper.mul1(val, tlen + 1, ints[0]);
		for(int j = 1; j < len; j++)
		{
			long borrow = 0;
	...
	public void p_square()
	{
		...
		if((tlen * 2) > this.length)
			this.build(tlen * 2);
		int max = val.length;
		MathHelper.mul1(val, tlen + 1, ref[0]);
	...
	public static int test()
	{
		...
		er += IPersistable.test(new VariableSInt("34123213123213123123123213213", 32));
		er += IDeepClonable.test(new VariableSInt("34123213123213123123123213213", 32));
		return er;
	}
